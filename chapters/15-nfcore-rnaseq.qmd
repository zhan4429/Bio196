...
- **`sample`:** A readable label for this library — letters, numbers, and underscores only. Use something like `CONTROL_REP1`, not `s1`.
- **`fastq_1`:** Path to the gzipped R1 FASTQ file. Use full absolute paths on shared systems.
- **`fastq_2`:** Path to the R2 file. Leave blank for single-end data.
- **`strandedness`:** One of `unstranded`, `forward`, `reverse`, or `auto`. When in doubt, use `auto`.
...
- **`docker`:** Laptops and workstations with Docker Desktop or root access.
- **`apptainer`:** Most modern HPC clusters. This is the Singularity successor.
- **`singularity`:** Older HPC clusters still running the original Singularity.
- **`<institution>`** (e.g., `tufts`, `crick`): Sites with a pre-built nf-core configuration covering executor, queue, and container cache. Use this if your cluster has one.
...
- **`multiqc/multiqc_report.html`:** The QC summary across every sample and every tool. Open this first.
- **`pipeline_info/software_versions.yml`:** Exact versions of every tool the pipeline ran. Paste this directly into your methods section.
- **`pipeline_info/execution_report.html`:** Per-task wall time, CPU, memory, and exit status — useful for tuning resources or diagnosing failures.
- **`pipeline_info/execution_timeline.html`:** Visual timeline of task execution. Good for spotting bottlenecks.
- **`pipeline_info/execution_trace.txt`:** Machine-readable task trace, useful for scripting or auditing runs.
...
- **Raw and trimmed read QC** (`fastqc/`, `fastp/` or `trimgalore/`): Per-sample HTML reports, all rolled up into MultiQC.
- **Alignment outputs** (`star_salmon/` or similar): BAM and BAI files, alignment logs, and optional bigWig coverage tracks.
- **Quantification matrices** (`star_salmon/` and/or `salmon/`): Gene- and transcript-level counts, TPM values, and a `SummarizedExperiment` RDS object for R users.
- **Post-alignment QC** (`rseqc/`, `qualimap/`, `dupradar/`, `preseq/`): Strandedness checks, gene-body coverage, duplication patterns, library complexity estimates.
...
- **`Sample sheet validation failed`:** A required column is missing or the `strandedness` value is misspelled. Re-check the four required columns and their allowed values.
- **Pipeline cannot find FASTQ files:** You used relative paths. Switch to full absolute paths in the samplesheet.
- **STAR fails with exit code 137:** Out of memory. Raise `--max_memory`, request a larger compute node, or add a `withName: STAR_ALIGN` block in a custom config to increase memory.
- **`-resume` re-runs everything:** Something changed — inputs, parameters, or the `work/` directory location. Keep parameters identical between runs when resuming.
- **Strandedness mismatch warning in MultiQC:** The samplesheet value doesn't match what the pipeline inferred from the data. Confirm your library protocol, fix the samplesheet, and re-run.
- **Containers fail to pull on HPC:** Compute nodes often have no internet access. Pre-pull images on the login node, or use `nf-core pipelines download` to stage everything before submitting jobs.
