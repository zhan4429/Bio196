...
The previous chapter introduced nf-core as an ecosystem of community pipelines. This chapter walks through one of them end to end. The goal is not to revisit RNA-seq biology from the earlier chapter, but to show how the workflow-management ideas from the last two chapters (Nextflow processes, profiles, samplesheets, `-resume`, container engines, parameter files) come together when you actually launch a real community pipeline.

We use `nf-core/rnaseq` version `3.24.0` as the worked example because it is the most widely run nf-core pipeline and exercises every part of the standard run pattern: a CSV samplesheet, a container profile, a reference genome, a long list of optional parameters, and a rich MultiQC report at the end. Whatever pipeline you eventually run for your own work — `sarek`, `ampliseq`, `chipseq`, `atacseq`, `taxprofiler`, `scrnaseq` — the mechanics will look very similar.
...
At a high level, the pipeline checks inputs, runs FASTQ-level QC, optionally trims and decontaminates, performs alignment and/or pseudoalignment, builds gene- and transcript-level matrices, runs many post-alignment QC tools, and aggregates everything into one MultiQC report.
...
The pipeline does not replace biological judgment. It will not tell you whether your experimental design makes sense or whether a surprising result is real. For the science of RNA-seq — library prep, alignment vs. pseudoalignment trade-offs, normalization, differential expression with DESeq2 — return to the RNA-seq chapter earlier in this course reader.
...
Every nf-core pipeline ships with a tiny test dataset. The test profile answers one question: *can my environment run this pipeline at all?*
...
A parameter file is preferable to a long command line for three reasons: it is easy to share, easy to diff in version control, and easy to attach to a methods section as the literal record of how the run was configured.
...
After the run, `results_rnaseq/` will contain many subdirectories. Do not try to read everything. From a workflow-management perspective there are three artifacts that matter most:
...
Before handing results to a collaborator, walk through this list. It applies to almost any nf-core run, not just RNA-seq:
...
`nf-core/rnaseq` is a worked example of the standard nf-core run pattern: pin the version, write a samplesheet, pick a container profile, drop your parameters in a YAML file, run the test profile first, then launch the real run with `-resume` ready to use. The MultiQC report and `pipeline_info/` directory together form the workflow artifact you keep — they document what ran, with which tool versions, on which samples, and how each task performed.

The same skills transfer directly to every other nf-core pipeline. Once you can run `rnaseq` cleanly, you can run `sarek`, `chipseq`, `atacseq`, `ampliseq`, `taxprofiler`, or `scrnaseq` with very small adjustments to the samplesheet and parameter file.

For the biology — what the count matrices mean, how strandedness affects quantification, how to model differential expression — return to the RNA-seq chapter earlier in this course reader.
