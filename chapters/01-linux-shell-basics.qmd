...
If you are new to bioinformatics, the command line may seem like an unnecessary detour — why type commands when you could click buttons? The answer lies in the nature of biological data itself.
...
Nearly every bioinformatics tool — `bwa`, `samtools`, `GATK`, `DESeq2`, `Snakemake` — is designed to be run from the command line. Mastering the shell is not optional; it is the entry ticket to computational biology.
...
![The Unix filesystem tree: everything lives under the root `/`. Your home directory `~` is typically `/home/username`.](../images/01-filesystem-tree.png){fig-align="center" width="80%"}
...
The Unix filesystem is organized as a tree rooted at `/` (the "root" directory). Everything — files, programs, devices — lives somewhere under `/`.
...
|------|-------------|---------|
| Absolute | Always starts from root `/` | `/home/user/data/genome.fa` |
...
The `$PATH` variable is particularly important — it is a colon-separated list of directories the shell searches when you type a command. If a tool is not found, it is usually because its directory is not in your `$PATH`.
...
Bioinformatics beginners often master `grep` before thinking to ask a more pressing question: "Am I about to fill the disk?" Sequencing data and intermediate files are large, so disk and process checks are everyday shell skills.
...
The **pipe** (`|`) sends the stdout of one command directly into the stdin of the next — without writing any intermediate files to disk. This is the essence of the Unix philosophy in action.
...
1. Reads a GFF annotation file

2. Filters to exon features only

3. Extracts the chromosome column

4. Sorts the chromosomes alphabetically

5. Counts unique chromosomes
...
Two shell features will dramatically speed up your work:
...
Bioinformatics jobs can take hours or days. If your network connection drops or your laptop closes, a running job in a plain terminal is killed immediately. **tmux** (terminal multiplexer) solves this by running your shell sessions inside a persistent server — you can detach, close your laptop, reconnect hours later, and find everything exactly as you left it.
...
The essential workflow: **create a named session → run your job → detach → reattach later**.
