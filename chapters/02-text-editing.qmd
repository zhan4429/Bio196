...
The critical concept: `vim` operates in **modes**. When you first open a file, you're in **Normal mode**, where keys are commands, not characters. You must explicitly enter **Insert mode** to type text.
...
New users type text in Normal mode and trigger random commands. The file fills with unexpected characters, or worse, gets deleted. If `vim` ever looks wrong, press `Esc` repeatedly until you're sure you're in Normal mode, then type `:q!` to exit without saving and start over.
:::
...
Visual Studio Code (usually called **VS Code**) is a free, open-source editor from Microsoft that's become the most popular coding environment in software engineering. For bioinformaticians, it offers three standout features:
...
Download VS Code from [code.visualstudio.com](https://code.visualstudio.com) for macOS, Linux, or Windows.
...
Opening a project folder shows you four main regions:
...
If you haven't already, add your server to `~/.ssh/config` on your local machine:
...
Replace `hpc.university.edu`, `myusername`, and the key path with your actual values. If you haven't set up SSH key authentication, do that first (it avoids typing your password on every reconnect):
...
Remote-SSH doesn't work reliably with password authentication on clusters requiring two-factor authentication (e.g., Duo). Set up SSH key authentication and, if your cluster requires it, configure SSH `ControlMaster` to multiplex connections:
...
This keeps one connection alive for 10 minutes, so repeated operations (opening files, running extensions) don't each require a new authentication step.
:::
...
VS Code lets you run Python interactively in two ways: open Jupyter notebooks (`.ipynb`) and run cells with `Shift+Enter`, or add `# %%` comment markers to any `.py` file to define "cells" and run them the same way. The second approach keeps your code in a plain `.py` file — version-control friendly and pipeline-compatible.
...
The **R extension** (`REditorSupport.r`) provides syntax highlighting and code completion. For full interactive use, add `radian`, a modern R console with multi-line editing and autocomplete:
...
Commit `.vscode/extensions.json` to help collaborators install the right extensions with one click. Be more cautious with `.vscode/settings.json`, as it may contain local paths (like the Python interpreter path) that differ between machines. Use a `.gitignore` entry or keep only portable settings in the committed version.
:::
...
- For a quick edit on a remote server with no graphical interface, use `nano`, or `vim` if you're comfortable with it.
...
Don't fall into the trap of only knowing graphical editors. Many HPC clusters don't allow graphical connections. A bioinformatician who can quickly edit a SLURM script in `vim` or `nano` without panicking is far more self-sufficient than one who needs VS Code for every edit. Invest a few hours in `nano` basics and `vim` survival skills—you'll use them for the rest of your career.
:::

## Summary

- `nano` is easy, always available, and best for quick edits on remote machines.
- `vim` is ubiquitous, modal, and keyboard-driven—worth learning for remote editing and server work.
- VS Code is graphical and extensible, making it the best day-to-day editor for larger projects.
