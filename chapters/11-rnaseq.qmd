...
**Genome alignment** (STAR or HISAT2, then featureCounts or HTSeq) aligns reads to the full genome and counts overlaps with annotated genes. It's slower but gives you BAM files you can open in IGV — useful when you suspect a library problem or want to verify a surprising result. **Transcriptome quantification** (Salmon or kallisto, imported with tximport) skips the alignment step and assigns reads directly to transcript sequences. It's much faster and handles multi-mapping reads more carefully, but depends on your reference transcriptome being complete and correct.
...
- **Low mapping rate** — often the wrong organism, contamination, low quality reads, or unadapted adapters. Check with FastQC; try Kraken or FastQ Screen; verify your reference genome version.
- **Low assigned reads** — wrong annotation, wrong strandedness, or too many intronic reads. Check the featureCounts summary file and confirm chromosome names match between your GTF and genome.
- **PCA separates by batch** — library prep or sequencing batch effect. Review your metadata, check sample processing records, and update your design formula.
- **One replicate is an outlier** — possible sample swap, failed library, or contamination. Look at MultiQC, alignment logs, and the sample distance heatmap together.
- **Many mitochondrial or rRNA reads** — RNA degradation or incomplete rRNA depletion. Check QC reports and run biotype-level summaries.
- **No significant genes** — low statistical power, weak biological effect, high variance, or the wrong contrast. Check replicate count, the dispersion plot, and that your contrast is specified correctly.
- **Thousands of genes significant** — possibly real biology, but also check for confounding, composition bias, or a normalization problem. MA plot and `plotDispEsts` are good starting points.
