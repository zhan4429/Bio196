...
Modern bioinformatics often outgrows a laptop. A whole-genome sequencing dataset can be
hundreds of gigabytes, RNA-seq projects commonly contain dozens of FASTQ files, and tools
such as STAR, genome assemblers, and deep-learning models may need more memory, CPU cores,
or GPUs than a personal computer has.

The Tufts High-Performance Computing (HPC) cluster is the shared research computing
system for Tufts students, staff, and researchers. It provides login nodes, compute nodes,
shared storage, software modules, and the Slurm scheduler so many people can run large
computing jobs without competing directly on the same machine.
...
This chapter focuses on the command-line route because it is what you will use most often
for reproducible bioinformatics workflows.

## What Is SSH?

**SSH** stands for Secure Shell. It is the standard way to open a secure terminal session
on a remote computer. When you use SSH, commands you type on your laptop are sent through
an encrypted connection to the remote server.
...
The first time you connect, SSH may ask whether you trust the host. After that, you will
be prompted for your Tufts password and any required 2FA step. A successful login drops
you onto one of the Tufts HPC login nodes, with a prompt similar to:
...
**SFTP** stands for SSH File Transfer Protocol. It uses the same secure connection as SSH,
but instead of giving you a remote shell, it gives you a small file-transfer prompt.
...
`rsync` also uses SSH, but it is better for real project work because it can copy whole
directories, show progress, and resume partial transfers. Run `rsync` from your laptop,
...
Tufts HPC storage is shared across the login and compute nodes. Once a file is copied to
cluster storage, it is visible from the node where your Slurm job runs.
...
Tufts home directories are fixed-size and intended for modest storage. If you are working
with a lab, use the lab project storage path for larger datasets. To check project storage
usage:
...
On your laptop, a command starts immediately. On a shared cluster, you request resources
and Slurm decides where and when your job runs.
...
Interactive sessions are useful for testing commands before writing a full batch script.
From a login node:
...
When Slurm allocates resources, your prompt changes to a compute node. Now it is safe to
run test commands. When you are done:
...
Tufts HPC uses environment modules to provide installed software. The exact software list
changes over time, so search before loading:
...
The scheduler can only allocate resources effectively if your request accurately reflects what your job needs.
...
Many bioinformatics tasks repeat the same command for many samples. Slurm job arrays are
...
This example runs FastQC on paired FASTQ files in a course project directory. Adjust paths
for your own username and data.
...
If the prompt says `login-prod`, stop and request a compute node. Use `sbatch` for batch
jobs or `srun --pty bash` for interactive testing.

### Using the Wrong Path

Use full Tufts HPC paths when transferring data:
...
Large requests can sit in the queue longer. Start with a small test, inspect `sacct` or
`seff`, then adjust.
