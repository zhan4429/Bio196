...
Understanding Docker's components will help you reason about what happens when you run commands.
...
An image is built once and can spawn many containers. When a container is deleted, any changes made inside it are lost — unless you explicitly save data to a mounted volume.

### Image Naming Convention

Docker images follow a naming scheme:
...
These two instructions control what happens when a container starts. The difference matters:

- **CMD**: provides a default command that can be completely overridden by the user.
- **ENTRYPOINT**: sets a fixed executable; user arguments are *appended* to it.
...
For bioinformatics, starting from a Conda base image is often the fastest path to a working container:
...
Bioinformatics tools often require compilers, development headers, and build tools that are not needed at runtime. A **multi-stage build** uses one stage to compile software and a second, smaller stage to hold only the final binaries:
...
Following these practices produces smaller, faster, more reproducible images:

1. **Combine `RUN` commands with `&&`**. Each `RUN` creates a layer. Combining related commands into a single `RUN` reduces layers and image size — especially when you install packages and then clean up caches in the same step.
...
2. **Order instructions from least to most frequently changing**. Docker caches layers from top to bottom. Put stable layers (OS packages) early and volatile layers (your scripts) late, so rebuilds reuse the cached base.

3. **Use `--no-install-recommends`** with `apt-get` to skip optional packages and keep the image lean.

4. **Pin all versions** — base images, packages, tools. Unversioned installs will silently change over time.

5. **Do not store data in images**. Mount data at runtime with `-v`. Images should contain only software.
...
[BioContainers](https://biocontainers.pro/) is a community-driven project that provides ready-to-use container images for thousands of bioinformatics tools. Every package in [Bioconda](https://bioconda.github.io/) is automatically built into both a Docker image and a Singularity image.

### Finding Images

The BioContainers registry hosts images on two main registries:

| Registry | URL Pattern | Example |
|----------|------------|---------|
| **Quay.io** | `quay.io/biocontainers/<tool>:<tag>` | `quay.io/biocontainers/samtools:1.20--h50ea8bc_1` |
| **Docker Hub** | `biocontainers/<tool>:<tag>` | `biocontainers/samtools:v1.9-4-deb_cv1` |

Browse available images at [biocontainers.pro](https://biocontainers.pro/) or search from the command line:
...
Some bioinformatics setups require multiple services — for example, a Galaxy server with a database, or a Jupyter notebook with a connected BLAST server. **Docker Compose** defines multi-container applications in a single YAML file.
...
Docker Compose is particularly useful for setting up reproducible analysis environments that combine a notebook interface with bioinformatics databases or services.
...
When you do not have registry access (common on HPC clusters), you can export and import images as tar files:
...
Both Snakemake and Nextflow have native support for Docker containers, making it straightforward to run each pipeline step in an isolated environment.
